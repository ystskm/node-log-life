...
(function() {
  ...
  function Life(filepath, options) {
    ...
    life.ready = Promise.resolve().then(function() {
      ...
      return life.renewalStreams(); // Wait creating symlink because removing real-file at the next step.
    }).then(function() {
      return; // DONNOT /dev/null for reboot process (cannot inherit fd)
      if(!is_new_fd) {
        return;
      }
      return new Promise(function(rsl, rej) { 
  ...
  function maintain(procTime) {
    ...
    return Promise.resolve(life.ready).then(function() {
      ...
      return life.renewalStreams(TRUE); // also renewal symlink 
    }).then(function() {
      // 1) Create backup gz
      // (1-1)
      return new Promise(function(rsl, rej) {
  ...
  function renewalStreams(symlink) {
    ...
    return Promise.resolve().then(function() {
      ...
      return new Promise(function(rsl, rej) {
        ...
        if(size > 0) {
          rwait = new Promise(function(rsl) {
            
            rext = Buffer(time.toGMTString()  + ' - [LogLife] Close write stream.');
            fs.readSync(life.fd, rbuf = Buffer(size), 0, size, life.offsetPosition);
            fs.write(life.ws, Buffer.concat([ rbuf, rext ]), rsl); 
        ...
        return new Promise(function(rsl, rej) {
          ...
          life.readTimer = setInterval(function() {
            ...
            fs.read(life.fd, buf, 0, once, life.offsetPosition, function(er, byteRead) {
              ...
              fs.write( life.ws, buf.slice(0, byteRead) );
  ...
  function createWriteStream(symlink) {
    ...
    return Promise.resolve().then(function() {
      ...
      return new Promise(function(rsl, rej) {
        ...
        fs.open(wp, 'w+', function(er, fd) { er ? rej(er): rsl(FDs[ wp ] = life.ws = fd); });
