...
function maintain(now) {
  ...
  return Promise.resolve().then(function() {
    ...
    // (flush log-file)
    // # truncate log file failure only occurs MongoDB log file.
    // for NOT standard out/err file descriptor?
    var fp_bk = fp + '.bk';
    var fp_trash = fp + '.trash';
    if(fp.indexOf('db/log') != -1) {
    ...
    function linux() {
      ...
      // (1) move filename 
      //   => file descriptor changes the point
      // new file is used when reopen by the "logLotate" command.
      cp.execSync('cp ' + fp + ' ' + fp_bk);
      fs.renameSync(fp, fp_trash);
      cp.execSync('touch ' + fp);

      // (2) unlink existing log
      //   find /proc/*/fd -ls | grep xxx
      //   => fd state will be changed "(deleted)"  and not logging any more.
      fs.unlinkSync(fp_trash);
