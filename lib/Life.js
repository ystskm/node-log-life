...
(function() {
  ...
  process.on('exit', function() {
    Lifes.forEach(function(life) {
      if(life.rs) try { life.rs.close(); } catch(e) { /*IGNORE*/ };
      if(life.ws) try { life.ws.close(); } catch(e) { /*IGNORE*/ };
    });
    Object.keys(FDs).forEach(function(fp) {
      try { fs.closeSync( FDs[fp] ); } catch(e) { outLog('Cannot close fd on exit for:' + fp); }
    });
  });
