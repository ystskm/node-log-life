...
function maintain(now) {
  ...
  return Promise.resolve().then(function() {
    ...
    // flush log-file
    // TODO for Windows logic
    var fp_trash = fp + '.trash';
    
    fs.renameSync(fp, fp_trash); // move filename
    cp.execSync('touch ' + fp); // make empty file

    fs.unlinkSync(fp_trash); // unlink existing log
    fs.linkSync(fp, fp_trash); // create redirect for existing process
    cp.execSync('rm ' + fp_trash); // and remove the link
