  ...
  function renewalStreams(symlink) {
    ...
    return Promise.resolve().then(()=>{
      ...
      return new Promise((rsl, rej)=>{
        ...
        let time = new Date();
        let size = fs.fstatSync(life.fd).size - (life.offsetPosition || 0);
        let smax = 10 * Math.pow(1024, 2);
        let rbuf, rext;
        if(size > smax) {
    ...
    return new Promise((rsl, rej)=>{
      ...
      const once =()=>{
        ...
        if(cur_size < cut_size) {
          // trancate 完了確認条件
          outLog(`(resolve) Comfirmed trancated on confirmTrancate: cut_size=${cut_size}, cur_size=${cur_size}`);
          return rsl();
        }
